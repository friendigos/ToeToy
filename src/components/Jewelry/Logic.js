...
function pdt360DegViewer(id: string, n: number, p: string, t: string, playable: boolean, autoPlay: boolean, draggable: boolean, mouseMove: boolean, buttons: boolean, keys: boolean, scroll: boolean): void {
    ...
    call++;
    loaderNone(id);
    var i: number = 1, j: number = 0, move: number[] = [],
        mainDiv: HTMLElement = document.querySelector(`#${id}`);
    ...
    var img: HTMLElement = document.querySelector(`#${id} .${id}`);
    ...
    if (!playable && !autoPlay) {
        ...
        function logic(el: HTMLElement, e: MouseEvent | TouchEvent) {
            j++;
            var x = touch ? (e as TouchEvent).touches[0].clientX : (e as MouseEvent).clientX;
        ...
        var interval: number, playing: boolean = false,
            pause: boolean = false,
            left: boolean = false,
            right: boolean = false,
            speed: number = 50;
